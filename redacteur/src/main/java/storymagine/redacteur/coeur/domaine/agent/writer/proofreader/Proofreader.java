 ...
 */
public class Proofreader implements Agent {
    ...
    private static final String AGENT_NAME = "Proofreader";
    ...
    public Proofreader(ModelCallPort llm) {
        this.llm = llm;
    }
    ...
    private List<ProofreaderOutput.Correction> parseCorrections(String response) {
        if (response == null || response.trim().startsWith("PAS D'ERREUR")) return List.of();
        List<ProofreaderOutput.Correction> result = new ArrayList<>();
        ...
        for (String line : lines) {
            ...
            if (t.startsWith("- FAUX:") || t.startsWith("FAUX:")) {
                ...
                if (!wrong.isBlank() && !correct.isBlank())
                    result.add(new ProofreaderOutput.Correction(wrong, correct));
                wrong = null;
            }
        }
        return result;
